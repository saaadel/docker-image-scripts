...
## HOW TO USE:
# curl https://raw.githubusercontent.com/saaadel/scripts/master/linux/posix-clean-update-install.sh | sh /dev/stdin [package1 [package2...]]
...
if [[ -n $IS_YUM_PKG_MANAGER ]]; then
    curl https://raw.githubusercontent.com/saaadel/scripts/master/linux/yum/clean-update-install.sh | sh /dev/stdin $*
else
    if [[ -n $IS_APT_GET_PKG_MANAGER ]]; then
        curl https://raw.githubusercontent.com/saaadel/scripts/master/linux/apt-get/clean-update-install.sh | sh /dev/stdin $*
    fi
fi
